...
#
# Same log-tee + GUI-error-on-crash pattern as Start VJ.sh — when the
# file manager launches this with "Execute" instead of "Execute in
# Terminal" there's no console for errors to surface to, so we capture
# everything to vj_last_run.log and pop a dialog on non-zero exit.
