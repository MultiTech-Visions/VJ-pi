...
float lineField(vec2 p, float a, float freq, float phase) {
    ...
    float v = sin(dot(p, d) * freq * exp2((param_x - 0.5) * 2.0) + phase);
...
void main() {
    ...
    float bands = sin((v + length(p) * 0.08 - time * 0.018) * (34.0 * exp2((param_x - 0.5) * 2.0)));
...
void main() {
    vec2 p = (v_texcoord - 0.5) * vec2(1280.0/720.0, 1.0) * exp2((param_x - 0.5) * 2.0);
    float field = 0.0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -2; x <= 2; x++) {
...
void main() {
    ...
    float irisHue = fract(0.58 + (param_x - 0.5) + time * 0.02);     // slow cool-tone drift; LEFT/RIGHT shifts eye colour
...
vec4 eye(vec2 p, vec2 c, float r, float seed) {
    ...
    float ri = 0.46;
    float hue = fract(0.07 + seed * 0.9 + (param_x - 0.5) + time * 0.02);   // warm, gold-ish, per eye; LEFT/RIGHT shifts the swarm's hue
...
void main() {
    vec2 p = (v_texcoord - 0.5) * vec2(1280.0/720.0, 1.0);
    float t = time;
    float zoom = (16.0 + sin(t * 0.13) * 5.0) * exp2((param_x - 0.5) * 2.0);     // breathing; LEFT/RIGHT scales the lattice
...
void main() {
    ...
    vec2 c = vec2(0.62 + 0.18 * sin(t * 0.13) + (param_x - 0.5) * 0.6, 0.74 + 0.14 * cos(t * 0.11) + (param_x - 0.5) * 0.6);
...
void main() {
    ...
    float fold = 1.05 + 0.18 * sin(t * 0.2) + (param_x - 0.5) * 0.5;     // animates the gasket; LEFT/RIGHT shifts packing density
    float scale = 1.0;
    float trap = 1e9;
    for (int i = 0; i < 8; i++) {
...
void main() {
    ...
    float pP = floor(5.0 + param_x * 3.0), qQ = 4.0;     // LEFT/RIGHT: {5..8, 4} tilings
    float wedge = PI / pP;
...
float DE(vec3 z) {
    float scale = 2.0 + (param_x - 0.5) * 0.6;     // LEFT/RIGHT morphs the fold scale
...
void main() {
    vec2 z = (v_texcoord - 0.5) * vec2(1280.0/720.0, 1.0) * 2.4 * exp2((param_x - 0.5) * 2.0);
    float t = time;
...
void main() {
    vec2 uv = (v_texcoord - 0.5) * vec2(1280.0/720.0, 1.0);
    vec2 start = uv * (2.5 * exp2((param_x - 0.5) * 2.0));
    vec2 q = start;
    float dens = 0.0;
    for (int i = 0; i < 12; i++) {
        q += curl(q) * 0.09;
...
void main() {
    vec2 uv = (v_texcoord - 0.5) * vec2(1280.0/720.0, 1.0);
    float t = time;
    float golden = 2.39996323 + 0.025 * sin(t * 0.1) + (param_x - 0.5) * 0.06;   // parastichy morph; LEFT/RIGHT nudges the divergence angle
...
void main() {
    ...
    float gsc = exp2((param_x - 0.5) * 2.0);     // LEFT/RIGHT: granulation scale
    vec2 warp = vec2(fbm(p * 3.0 * gsc + vec2(t * 0.05, -t * 0.04)),
                     fbm(p * 3.0 * gsc + vec2(-t * 0.045, t * 0.05)));
    float gran = fbm(p * 4.0 * gsc + warp * 1.6 + t * 0.03);
...
void main() {
    ...
    for (int k = 0; k < 6; k++) {
        ...
        float petals = (6.0 + fk * 2.0) * exp2((param_x - 0.5) * 1.5);
...
void main() {
    ...
    float N = floor(3.0 + param_x * 6.0);     // LEFT/RIGHT: 3..9 mirror segments
...
void main() {
    ...
    float ct = time * 0.13;
    float rad = (2.7 + 0.7 * sin(time * 0.06)) * exp2((0.5 - param_x) * 1.2);     // LEFT/RIGHT zooms the camera in/out
...
# Every GPU generator exposes two arrow-key knobs plus a speed-controlled
# animation clock, without each shader having to declare the plumbing by
# hand. We rewrite each catalogue entry to:
#   * rename the built-in `time` clock to `atime` — the worker integrates
#     this at an operator-controlled rate (UP/DOWN = speed) so a generator
#     can be sped up or slowed without the pattern jumping; and
#   * declare the `param_x` (LEFT/RIGHT) and `param_y` (UP/DOWN) uniforms
#     that shader bodies read for their per-generator "character" knob.
# Shader bodies reference `atime` / `param_x` / `param_y` directly; only the
# declarations are injected here, so the catalogue above stays uncluttered.
def _tunable(src):
    src = re.sub(r"\btime\b", "atime", src)
    return src.replace(
        "uniform float atime;",
        "uniform float atime;\n"
...
GPU_GENERATORS = {name: _tunable(src) for name, src in GPU_GENERATORS.items()}
